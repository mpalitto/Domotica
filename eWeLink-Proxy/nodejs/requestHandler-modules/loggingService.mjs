...
UPDATED: All log files are now stored in the logs/ subdirectory
*/

import { appendFileSync, renameSync, existsSync, mkdirSync } from 'fs';
import { debugMode, protocolCapture, sONOFF, LOGS_DIR } from '../sharedVARs.js';
...
export class LoggingService {
    ...
    static debugLog(deviceID, direction, type, data) {
        if (debugMode.enabled && debugMode.deviceId === deviceID) {
            ...
            try {
                appendFileSync(`${LOGS_DIR}/${deviceID}.debug`, logEntry);
            } catch (err) {
    ...
    static enableDebugForDevice(deviceID, reason) {
        originalConsoleLog(`\n${'🔍'.repeat(40)}`);
        originalConsoleLog(`🔍 AUTO-ENABLING DEBUG MODE FOR DEVICE: ${deviceID}`);
        originalConsoleLog(`   Reason: ${reason}`);
        originalConsoleLog(`   Debug file: logs/${deviceID}.debug`);
        originalConsoleLog(`${'🔍'.repeat(40)}\n`);
        
        // Rotate existing debug file to .bak
        const debugFile = `${LOGS_DIR}/${deviceID}.debug`;
        const bakFile = `${LOGS_DIR}/${deviceID}.debug.bak`;
        ...
        if (existsSync(debugFile)) {
            try {
                ...
                originalConsoleLog(`📁 Rotated old debug file to logs/${deviceID}.debug.bak`);
            } catch (err) {
                originalConsoleLog(`⚠️  Could not rotate debug file:`, err.message);
            }
        }
...
`;
        ...
        try {
            ...
            originalConsoleLog(`⚠️  Could not write to debug file:`, err.message);
        }
    ...
    static disableDebugForDevice(deviceID) {
        if (debugMode.enabled && debugMode.deviceId === deviceID) {
            originalConsoleLog(`\n🔍 Disabling debug mode for ${deviceID}\n`);
...
`;
            ...
            try {
                appendFileSync(`${LOGS_DIR}/${deviceID}.debug`, footer);
            } catch (err) {
                originalConsoleLog(`⚠️  Could not write footer to debug file:`, err.message);
            }
        ...
        console.log = (...args) => {
            ...
            if (debugMode.enabled && debugMode.deviceId) {
                ...
                if (isRelevant) {
                    ...
                    if (message.includes('─'.repeat(10)) || 
                        message.includes('═'.repeat(10)) ||
                        message.includes('🔌') || 
                        message.includes('📋') ||
                        message.includes('✅') ||
                        message.includes('❌')) {
                    ...
                    try {
                        appendFileSync(`${LOGS_DIR}/${debugMode.deviceId}.debug`, logEntry);
                    } catch (err) {
                        originalConsoleLog('⚠️  Error writing console output to debug file:', err.message);
                    }
                }
            }
        };
     ...
     */
    static debugSeparator(deviceID, char = '─', length = 80) {
        if (debugMode.enabled && debugMode.deviceId === deviceID) {
            const separator = `\n${char.repeat(length)}\n`;
            try {
                appendFileSync(`${LOGS_DIR}/${deviceID}.debug`, separator);
            } catch (err) {
